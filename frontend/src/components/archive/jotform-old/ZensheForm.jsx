  ...
  return (
    <div className="zenshe-form-container" style={customStyles}>
      ...
      <div style={{
        ...
        ⚠️ IMPORTANT: Remplissez le formulaire mais N'APPUYEZ PAS sur le bouton dans le formulaire!
        <br />
        Utilisez le bouton "📤 Envoyer à la Praticienne" en bas à la place.
      </div>
        ...
        <div className="zenshe-form-content">
          <iframe
            ...
            onLoad={() => {
              ...
              // Hide JotForm's submit button so user can't submit directly
              const iframe = document.getElementById(`JotFormIFrame-${FORM_ID}`);
              if (iframe) {
                try {
                  const iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
                  if (iframeDoc) {
                    // Inject CSS to hide all submit buttons in the form
                    ...
                    console.log('✅ JotForm submit buttons hidden');
                  }
                } catch (err) {
                  console.warn('⚠️ Cannot hide submit button (cross-origin):', err.message);
                  console.log('💡 User must NOT click JotForm submit button - use external button only');
                }
              }
            }}
            allowTransparency="true"
            allow="geolocation; microphone; camera; fullscreen"
            src={`https://form.jotform.com/${FORM_ID}?noCaptcha=true&isCardForm=1`}
            frameBorder="0"
            style={{
              minWidth: '100%',
              maxWidth: '100%',
